...
import { TaskEndHistoryItem } from "../History/HistoryItems/Task/Properties/TaskEndHistoryItem";
import { TaskMoveHistoryItem } from "../History/HistoryItems/Task/Properties/TaskMoveHistoryItem";
import { TaskProgressHistoryItem } from "../History/HistoryItems/Task/Properties/TaskProgressHistoryItem";
import { TaskStartHistoryItem } from "../History/HistoryItems/Task/Properties/TaskStartHistoryItem";
    ...
    public moveEndDependTasks(predecessorTaskId: string, predecessorPreviousEndDate: Date, isAfterCorrectParents: boolean = false): void {
        ...
        dependencies.forEach((dependency) => {
            ...
            if(!successorRangeBeforeMove.equal(validTaskRange)) {
                this.history.addAndRedo(new TaskMoveHistoryItem(this.modelManipulator, dependency.successorId, validTaskRange));
    ...
    public updateParentsRangeByChild(taskId: string): void {
        this.recalculateParents(this.viewModel.findItem(taskId), (data: any) => {
            ...
            if(isDefined(data.start)) {
                history.addAndRedo(new TaskStartHistoryItem(manipulator, data.id, data.start));
                this.moveStartDependTasks(data.id, data.oldStart);
            }
            if(isDefined(data.end)) {
                history.addAndRedo(new TaskEndHistoryItem(manipulator, data.id, data.end));
                this.moveEndDependTasks(data.id, data.oldEnd);
            }
            if(isDefined(data.progress))
                history.addAndRedo(new TaskProgressHistoryItem(manipulator, data.id, data.progress));
        });
    }
    ...
    public updateChildRangeByParent(parentId: string, delta: number, changedTasks: Array<any>): void {
        ...
        for(let i = 0; i < children.length; i++) {
            ...
            if(newStart < childTask.end) {
                this.history.addAndRedo(new TaskStartHistoryItem(this.modelManipulator, childTask.internalId, newStart));
                this.history.addAndRedo(new TaskEndHistoryItem(this.modelManipulator, childTask.internalId, newEnd));
            }
            else {
                this.history.addAndRedo(new TaskEndHistoryItem(this.modelManipulator, childTask.internalId, newEnd));
                this.history.addAndRedo(new TaskStartHistoryItem(this.modelManipulator, childTask.internalId, newStart));
