...
export class GanttView implements IGanttView {
    ...
    setTaskValue(id: string, fieldName: string, newValue: any): boolean {
        ...
        if(task) {
            if(fieldName === "title") {
                const checkedNewValue = newValue ? newValue : "";
                return manager.changeTaskTitleCommand.execute(task.internalId, checkedNewValue);
            }
            if(fieldName === "progress") {
                let newProgress = Math.max(newValue, 0);
                newProgress = Math.min(newValue, 100);
                return manager.changeTaskProgressCommand.execute(task.internalId, newProgress);
            }
            if(fieldName === "start")
                return manager.changeTaskStartCommand.execute(task.internalId, DateTimeUtils.getMinDate(newValue, task.end));
            if(fieldName === "end")
                return manager.changeTaskEndCommand.execute(task.internalId, DateTimeUtils.getMaxDate(newValue, task.start));
        }
        return false;
    }
